...
`#include`最常见的用途，就是用来加载包含函数原型的头文件（后缀名为`.h`）。多个`#include`指令的顺序无关紧要，多次包含同一个头文件也是合法的。
...
除了编写C/C++混合代码时用到，`__cplusplus`也常用于判断编译器支持的C++标准，如C++03标准中，`__cplusplus`值被预定义为199711L，C++11标准中，`__cplusplus`被预定义为201103L。
